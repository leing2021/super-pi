...
## ⛔ Iron Laws (override any feature design)

- **Law 1: Reduce burden, never add it.** Any design that increases user operation complexity — an extra confirmation, an extra manual handoff, an extra required parameter — is a design violation; do not merge. Before adding anything, answer: does the user take more steps or fewer?
- **Law 2: Minimalism — nothing unnecessary.** The default path has zero extra operations; any additional interaction must have an irreplaceable reason.
- **Acceptance is measured by live runs.** Any claimed "automatic / continuous / recommended" behavior must be verified to actually happen (e.g. 03-work truly chains into 04-review then 05-learn). Written in docs ≠ happens in reality; unverified automation is unimplemented.
