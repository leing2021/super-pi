...
- 新增 5 字段 YAML frontmatter：title, category, severity, tags, applies_when
- `02-plan` 和 `04-review`：grep-first 搜索策略（项目级 + 全局级 `~/.pi/agent/docs/solutions/`）
- `05-compound`：强制 frontmatter，两级存储（项目特定 vs 跨项目全局）
- 新增 `solution-search-strategy.md` 参考文件
- 95 个测试全部通过
