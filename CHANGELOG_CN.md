...
- **Fix loop 语义重写**：cap 2 轮为上限非配额（任一轮全绿提前终止）；重审触发 = 存在 P0 或上轮 P1 有待验修复 diff，纯 P2 不进 loop；重审为增量 spawn（上轮 findings + 修复 diff）；cap 耗尽仍有 P0/P1 → 停线交付末轮 findings + 完整 findings chain。
- **降级契约机器可读**：spawn 失败四分类（`cli_not_found` / `version` / `timeout` / `child_failed`）降级为同 session 审查，findings frontmatter 写 `isolation: degraded` + `spawn_error`——下游可辨识低置信证据；确定性失败直接降级，超时重试 1 次再降级；degraded 下 loop 语义不变。
- **Live 验收**：埋雷 diff（off-by-one）隔离审 89.1s 抓到 P0；本 feature 自审 3 轮实证全部机制——round 1 抓 6 findings、round 2 增量审抓到修复引入的回归（Bun 专属 `import.meta.dir` 在 pi Node+jiti 运行时全灭 12 工具、bun test 全绿掩盖）、cap 停线 valve 触发、round 3 全绿提前终止。修复：`import.meta.dirname`（三运行时安全）、stderr tail 诊断、AbortSignal 透传。
- 测试：236 通过（967 断言）+19；`tsc --noEmit` 干净；pi 真实加载验证通过。
