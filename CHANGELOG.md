...
- **isolated_review live progress** (`extensions/ce-core/tools/isolated-review.ts`, `index.ts`): the tool blocked 2-5 minutes with zero visible output while its spawned reviewer session ran, and an abort was the only exit that wrote no findings and carried no reason. `onUpdate` is now forwarded through every layer (registerTool → tool.execute → runIsolatedReview → runOnce): a spawn notice with the expected duration lands immediately, and each assistant `message_end` from the reviewer streams a one-line preview into the host TUI.
- **Abort evidence on disk**: an aborted spawn now writes a findings file (`isolation: aborted` + elapsed time + reviewer stderr tail) instead of vanishing; the result carries `findingsWritten`/`elapsedMs` and the summary reads `aborted after Ns (abort signal); evidence at <path>`. Auto-degrade on abort was deliberately rejected — an interrupt is the user's valve, not a trigger to relaunch a second long-running review.
- Tests: 250 passing (1013 assertions) +7 (display-width/grapheme/mixed-width truncation, progress reporting, abort evidence, onUpdate forwarding + registration contract); `tsc --noEmit` clean. CHANGELOG.md history entries translated to English.

### 0.38.0 — ask_user_question selector UX: 1-9 quick pick, transcript peek, rich options
- **1-9 quick pick**: options carry `1. ` `2. ` prefixes; pressing the digit selects immediately (single-key `/^[1-9]$/` guard), no arrow-key grinding. Community contribution PR #16.
- **Ctrl+] transcript peek**: collapses the dialog to a one-line summary (question first line) so the user can re-read the agent's prior analysis in the terminal before answering; Esc restores the dialog instead of cancelling (prevents accidental context loss); the hint text leads to survive narrow terminals.
- **Rich options**: options accept `{ label, description }` objects, rendered as `label — description`; the LLM only receives the clean label. String options remain fully backward compatible.
- **Enter accepts `\r`** (some terminals send CR).
- Tests: 243 passing (992 assertions) +7 (digit keys, prefix rendering, collapse toggle/Esc restore, rich-options parsing); `tsc --noEmit` clean; merge verified clean.

### 0.37.0 — isolated review: 04-review spawns a fresh session, author bias eliminated
- **Isolated review** (`isolated_review` tool, ce-core): 04-review now first spawns a fresh pi session with zero author context (`pi --mode json --no-session -p`) to run the review, blocking until the findings artifact returns to the main chain. Self-consistency bias from same-session self-review is eliminated structurally, not by prompt discipline. Registered tools: 11 → 12.
- **Reviewer reviews, never fixes**: the spawned reviewer produces findings; fixes belong to the main session (the author has full implementation context). Identity constraints ship in the packaged prompt asset (`skills/04-review/assets/isolated-reviewer-prompt.md`): no code fixes, findings written to a given path, no chaining into 05-learn, full rules loading.
- **Fix loop semantics rewritten**: the 2-round cap is a ceiling, not a quota (any all-green round terminates early); re-review triggers only on a P0 or a P1 with a fix diff awaiting verification — pure P2 findings never re-enter the loop; re-review is an incremental spawn (previous findings + fix diff, not the whole codebase); cap exhausted with P0/P1 remaining → stop-the-line, hand over the latest findings plus the full findings chain.
- **Machine-readable degradation contract**: spawn failures (`cli_not_found` / `version` / `timeout` / `child_failed`) degrade to in-session review, writing `isolation: degraded` + `spawn_error` into the findings frontmatter — downstream stages (05-learn/handoff) can recognize low-confidence evidence. Deterministic failures degrade immediately; timeouts retry once, then degrade; loop semantics are unchanged under degradation.
- **Live acceptance (iron law: unverified = unimplemented)**: a planted off-by-one diff was caught as P0 in 89.1s of isolated review (with sandbox numeric verification); the feature's own 3-round self-review exercised every mechanism — round 1 caught 6 findings (including an npm cross-repo prompt-asset resolution bug), round 2's incremental review caught a regression introduced by the fix (Bun-only `import.meta.dir` killed all 12 tools under pi's Node+jiti runtime while `bun test` stayed green), the cap stop-the-line valve fired, and round 3 terminated early all-green. Fixes: `import.meta.dirname` (safe across all three runtimes), stderr tail diagnostics, AbortSignal pass-through.
- Tests: 236 passing (967 assertions) +19 (13 tool cases: success/four-way degradation/retry+stderrTail/incremental injection/abort/prompt source tri-state; contracts anchoring loop wording and reviewer identity constraints); `tsc --noEmit` clean; verified under real `pi` loading.

### 0.36.0 — pipeline auto-chaining: one entry to 05-learn, worktree off the main flow
- **Auto-chaining**: once a stage passes its gate, the next stage executes in the same session (read the next SKILL.md and run it): 01→02→…→05, with the four valves in force chain-wide. Only two confirmations remain: 01→02 requirements approval (pre-existing) and the 02→03 work gate (review choices folded in, `✓ 开工` preselected). 05-learn is the terminus and emits a final summary.
- **Fix loop closure**: 04-review P0/P1 findings are fixed and re-reviewed in the same round; two consecutive rounds still holding P0/P1 stops and asks (replacing the old max-3-iterations — this is now the repo's only cap).
- **Worktree cleared from the main flow**: 02-plan advisory, 03-work worktree rules, 06-next recommendation chains and references all removed; `07-worktree` remains a standalone skill triggered only by explicit `/skill:07-worktree`.
- **Iron laws codified** (AGENTS.md): Law 1 reduce burden, never add it; Law 2 minimalism — nothing unnecessary; acceptance is measured by live runs.
- Tests: 217 passing (904 assertions), +3 contract tests (chaining instruction presence, worktree absence, shared clauses) guarding against silent regression.
...
- **Recommended answer on every question** (`skills/01-brainstorm/SKILL.md`, `skills/02-plan/SKILL.md`): both skills now require exactly one option prefixed with the `✓ 推荐` (recommended) marker plus a one-line reason for every `ask_user_question` — a missing recommendation is a blocking violation. Previously the discipline lived only in `premise-challenge.md` (since 0.28.0), leaving main-flow questions (mode selection, CEO Review) without recommendations.
...
- **Continuous execution for 03-work** (`skills/03-work/SKILL.md`): no pauses between units — progress goes to `session_checkpoint`, report once at completion. Exactly four enumerated valves may stop and ask: (1) stop-the-line failure that cannot self-heal, (2) 3-failure cap on the same unit, (3) destructive or irreversible operations (delete, overwrite, force-push, no undo path), (4) spec ambiguity / plan gap — record blocker, ask, never guess. Design philosophy: the frontend (01/02) asks little but precisely; the backend (03) doesn't ask — unless a hard valve fires.
- **Worktree advisory at the decision point** (`skills/02-plan/SKILL.md`): when finalizing a plan with units ≥ 5 or files ≥ 8, 02-plan appends one advisory line — "Large task — recommended: run `/skill:07-worktree` first for isolation, then `/skill:03-work`." The hint lands exactly where the user decides whether to launch 03-work, with unlimited time to act, and states the recommended action sequence explicitly. (First implemented inside 03-work post-`task_splitter`, where it displayed and executed in the same breath — a hint with no time to act is noise; moved to the gate the user must cross anyway. Plan artifacts are often uncommitted, so auto-routing into a fresh worktree stays off the table.)
- **100-line guard rail fired and honored**: adding rules pushed 03-work SKILL.md to the ≤100-line contract limit; Error compaction section compressed 4→2 lines with all four points preserved. Guard rail not relaxed.
- **CEO Review proved its worth**: it caught a real transcription loss — the brainstorm's 4th valve was missing from the plan. Captured as a solution artifact (`docs/solutions/workflow/…`, local docs, not shipped).
- **Worktree git prerequisite** (`skills/07-worktree/SKILL.md`): `create` requires git — verify with `git rev-parse --is-inside-work-tree`; if it fails, tell the user why and stop.
- **Tests** (`tests/skill-contracts.test.ts`): +14 contract assertions anchoring the recommended-answer rules, the four-valve enumeration, and the advisory threshold. 219 tests passing, 904 assertions, 0 regressions.
...
- **done lands at 04-review, not 05-learn**: plan lifecycle ends at implementation acceptance; 05-learn's solution artifacts are a side-channel knowledge product. 04-review sits on the forced handoff chain (03 → 04) while 05-learn can be skipped — archive-on-skippable-stage is exactly how the 91-plan backlog happened. Rework stays consistent: 03-work flips the status back to `executing` automatically.
- **Architecture decisions heading is now mandatory** (`plan-template.md`): "No ADR-worthy decisions" must be stated under the existing heading instead of deleting it — 28/32 existing plans silently dropped the heading, breaking grep-based ADR audits.
- **Tests** (`tests/skill-contracts.test.ts`): +4 assertions anchoring the status header and closed vocabulary across 02-plan/03-work/04-review. Mutation-verified (reverting the 02-plan change turns the suite red). 219 tests passing, 888 assertions, 0 regressions.

### 0.32.0 — rules-loading enforcement: blocking steps, manifest, project-level language extension
- **Rules loading is now a blocking, visible step** (`skills/03-work/`, `skills/04-review/`): the "load project rules" instruction previously lived in Core-rules prose and was silently skipped in practice — agents went straight to implementing/reviewing without reading any rule files. It is now a numbered blocking workflow step ("no manifest, no implementation/findings") with an inline language map (no two-hop reference), and it must emit a visible one-line manifest (`Rules loaded: language=… (via …), common=…, lang=…, web=…`) before any code or finding. Users can now see whether rules actually loaded.
- **Same-session re-entry skips re-reading**: when the transcript already contains a matching `Rules loaded:` manifest, rule files are not re-read — repeated 03-work/04-review rounds in one conversation no longer pay the token cost twice. Cross-session reads are unchanged (new context has no manifest, re-reading is the necessary cost).
- **Dangling completion checklist fixed** (`skills/references/pipeline-config.md`): both skills referenced a "completion checklist in pipeline-config.md" that never existed. The section now exists (rules manifest / workflow steps / artifacts / verification evidence / status blocks), scoped so stages without a rules-loading step aren't force-failed. A new contract test locks reference integrity — the dangling reference had survived multiple releases precisely because nothing asserted it.
- **Project-level language extension point** (`skills/references/language-detection.md`, both skills): projects can support languages outside the built-in 13 by adding `{repo-root}/rules/<lang>/` plus a marker row in `{repo-root}/rules/language-detection.md`. Merge semantics: append + same-marker-wins, consistent with the existing rules override chain. Malformed rows fall back visibly (manifest shows mapping source; missing rules dirs surface as `lang: missing`) — never silently.
- **`rules/typescript/review-checklist.md` added**: TS/JS-specific defect patterns for 04-review (null/undefined, type safety, floating promises, async forEach, event-listener leaks, stale closures, NaN/equality) matching the python/golang checklist structure. 04-review now marks missing per-language checklists as `missing (fell back to common)` instead of skipping silently.
- **Docs synced**: `rules/README.md` (all 13 language dirs listed, install.sh reference removed — script never existed, end-user project-level path documented), both READMEs (12 common topics, extension point, test counts). Upstream ECC `rules/` diffed and evaluated: nothing absorbed (framework-level `fastapi.md` conflicts with whole-directory loading; quarkus refs dangle locally) — recorded in `docs/out-of-scope/` with a re-open condition.
- **Tests** (`tests/skill-contracts.test.ts`): +3 contract tests — manifest requirement + blocking steps + TS checklist presence, project-level extension docs, skill wiring. All RED-verified (breaking the target section fails the suite). 219 tests passing, 0 regressions.

### 0.31.0 — dual-trigger model routing: skill-read hook + aux stage keys
- **Model-initiated skill invocation now routes** (`extensions/ce-core/`): modelStrategy previously triggered only on explicit `/skill:` commands via the `input` hook. When users described the task in natural language and the model read a stage's `SKILL.md` on its own — the most common invocation path — routing silently skipped. New `tool_call` hook fires on reads matching `skills/<stage>/SKILL.md` (absolute, `./`-prefixed, bare-relative, and Windows paths), invoking the same routing logic. `setModel` lands on the LLM request after the read returns, so the whole skill execution runs on the target model. Same-model re-reads are idempotent (no duplicate switches/notifications).
- **Aux stages routable** (`extensions/ce-core/utils/stage-routing.ts`): `PIPELINE_STAGE_KEYS` now includes all 7 stages (01-05 pipeline + 06-next/07-worktree) — configuring `modelStrategy`/`thinkingStrategy` keys for them previously had no effect.
- **Routing extracted** to `extensions/ce-core/utils/stage-routing.ts` (228 lines); `index.ts` shrinks 860 → 718 lines (back under the 800-line file limit it had outgrown). Docs synced (`skills/references/pipeline-config.md`, both READMEs); solution artifact updated with the dual-trigger pattern (`docs/solutions/tooling/2026-04-24-pi-extension-terminate-and-model-routing.md`).
- **Tests** (`tests/ce-core-extension.test.ts`): +4 — 06/07 stage switching, SKILL.md read routing (absolute + bare-relative), non-skill-path negative. Regex boundaries verified against 9 path shapes. 216 tests passing, 0 regressions.
...
- **Documentation**: `03-work` returns to inline-first; all `pi-subagents` references removed from README/README_CN (minimalism).
